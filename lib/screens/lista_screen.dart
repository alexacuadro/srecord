...
import 'package:srecord/screens/bote_screen.dart';
import 'package:srecord/screens/login_screen.dart';
import 'package:srecord/screens/premios_screen.dart';
import 'package:srecord/screens/mis_partes_screen.dart';
import 'package:srecord/screens/numeros_limitados_screen.dart';
import 'package:srecord/screens/ajuste_topes_listero_screen.dart';
import 'package:srecord/screens/comunicados_screen.dart';
import 'package:srecord/screens/buzon_notificaciones_screen.dart';
...
class _ListaScreenState extends State<ListaScreen> with SingleTickerProviderStateMixin {
  ...
  Timer? _typingTimerRef;
  String _listeroName = "Cargando...";
  String _listeroPin = "";
  bool _isAnnouncementScreenOpen = false;
  ...
  void _checkSessionAutoJump() {
    ...
    // Detectar si la sección acaba de abrirse (Transición de bloqueado a visible)
    bool keyboardWasVisible = _isKeyboardVisible();
    ...
    if (_lastKnownOpenSession != null && 
      ...
      if (_activeSeccion == _lastKnownOpenSession!["seccion"] && 
        ...
        debugPrint("[S-RECORD] Salto automático de sesión detectado: ${currentOpen["seccion"]}");
        setState(() {
        ...
        // AVISO FÍSICO: Vibración y Sonido al abrir nueva sección
      ...
      // Si la sección no cambió pero el teclado se activó por horario (ej: dieron las 2:00 PM)
      if (!keyboardWasVisible && _isKeyboardVisible()) {
         HapticFeedback.heavyImpact();
         SystemSound.play(SystemSoundType.click);
         setState(() {}); // Refrescar para mostrar teclado
      }
    }
  }
  ...
  void _onSyncUpdate(int id) {
    if (mounted) {
      // Si el cambio vino de la nube (-999), recargar absolutamente todo (Sesión, Plan, Color, Tiro)
      if (id == -999) {
        _loadInitialConfig().then((_) {
          _refreshItems();
          _calculateTotalPremios();
  ...
  String _getCountdownText(DateTime now) {
    ...
    if (_activeLoteria == "GEORGIA") {
      ...
      } else if (time < RecaudacionService.timeNocheAbre) {
        targetMinutes = RecaudacionService.timeNocheAbre;
        label = "ABRE ";
      } else if (time < RecaudacionService.timeNocheCierre) {
        targetMinutes = RecaudacionService.timeNocheCierre;
      } else {
        targetMinutes = 1440; // Medianoche
        label = "ABRE ";
      }
    }
  ...
  Future<void> _loadListeroAndData() async {
    ...
    if (listeros.isNotEmpty && planes.isNotEmpty) {
      ...
      } catch (e) { debugPrint("Error plan: $e"); }
    }
    ...
    if (_listeroPin.isNotEmpty) {
      ...
      if (ptJson != null) {
        ...
        _personalTopes.putIfAbsent("bola_lista", () => _parseLimit(_currentPlan["tope_bola"]));
        _personalTopes.putIfAbsent("parle_lista", () => _parseLimit(_currentPlan["tope_parlet"]));
        _personalTopes.putIfAbsent("centena_lista", () => _parseLimit(_currentPlan["tope_centena"]));
        _personalTopes.putIfAbsent("bola_bote", () => _parseLimit(_currentPlan["tope_bote_bola"]));
        _personalTopes.putIfAbsent("parle_bote", () => _parseLimit(_currentPlan["tope_bote_parlet"]));
        _personalTopes.putIfAbsent("centena_bote", () => _parseLimit(_currentPlan["tope_bote_centena"]));
      } else {
        _personalTopes = {
          "bola_lista": _parseLimit(_currentPlan["tope_bola"]),
          "parle_lista": _parseLimit(_currentPlan["tope_parlet"]),
          "centena_lista": _parseLimit(_currentPlan["tope_centena"]),
      ...
      // AUTO-EXHIBICIÓN DE COMUNICADOS: Si hay algo pendiente, mostrarlo ahora.
      _checkPendingAnnouncements();
    }
    if (mounted) setState(() {});
  }
  ...
  Future<void> _checkPendingAnnouncements() async {
    if (_isAnnouncementScreenOpen) return;

    final comunicado = await Alex().getPendingComunicado();
    if (comunicado != null && mounted) {
      final navigator = Navigator.of(context);
      final prefs = await SharedPreferences.getInstance();
      final role = prefs.getString("user_role") ?? "LISTERO";
      
      setState(() => _isAnnouncementScreenOpen = true);
      
      // Mostrar como pantalla completa para obligar lectura
      navigator.push(
        MaterialPageRoute(builder: (_) => ComunicadosScreen(comunicado: comunicado, userRole: role)),
      ).then((_) {
        if (mounted) {
          setState(() => _isAnnouncementScreenOpen = false);
          _loadListeroAndData(); // Re-chequear al volver por si hay más
        }
      });
    }
  }
  ...
  Future<void> _calculateTotalPremios() async {
    ...
    List<Map<String, dynamic>> jugadas = await _db.getJugadasCompletas(_listeroPin, destino: 'LISTA', seccion: _activeSeccion, fecha: _activeFecha, bancoId: bancoId, loteria: _activeLoteria);
    ...
    if (parteOficial != null && parteOficial['publicado'] == 1) {
      _premiosTotal.value = (parteOficial['premios_lista'] as num).toDouble();
      _limpioTotal.value = (parteOficial['limpio_lista'] as num).toDouble();
      if (parteOficial['winners_json'] != null) {
        try {
          final decoded = json.decode(parteOficial['winners_json']);
          _itemPrizes.clear();
          for (var w in (decoded['lista'] ?? [])) {
    ...
    for (var j in jugadas) {
      ...
      double p = RecaudacionService.calculatePremio(j['tipo'], j['valor'], tiro, _currentPlan, 'LISTA', 
        customLimites: customLimites, 
        seccion: _activeSeccion,
        limpioTotal: _limpioTotal.value,
  ...
  void _scrollToHistoryTop(int field) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) { if (c.hasClients) c.jumpTo(0); });
  }

  void _scrollToEnd() {
    WidgetsBinding.instance.addPostFrameCallback((_) { if (_visorScrollController.hasClients) _visorScrollController.jumpTo(_visorScrollController.position.maxScrollExtent); });
  }
  ...
  void _notifyTyping() {
    if (!_isTypingStatus) {
      _isTypingStatus = true;
      Alex().broadcastTypingStatus(true);
    }
    _typingTimerRef?.cancel();
    _typingTimerRef = Timer(const Duration(seconds: 3), () {
      _isTypingStatus = false;
      Alex().broadcastTypingStatus(false);
    });
  }

  Future<double> _getAcumulado(String num, String subT, {String destino = "LISTA"}) async {
    final bancoId = await Alex().getActiveBancoId();
    List<Map<String, dynamic>> hist = await _db.getJugadas(_listeroPin, subT == "CENTENA" ? "CENTENA" : (subT == "PARLE" ? "PARLE" : "BOLA"), destino: destino, seccion: _activeSeccion, fecha: _activeFecha, bancoId: bancoId);
    double suma = 0;
    for (var j in hist) {
      String val = j['valor']; String numPart = val.split('(').first.trim();
      if (subT == "PARLE") {
  ...
  void _addRegistro() async {
    if (_isReadOnly() || _isProcessing) return;
    ...
    if (pIdx != -1 && aIdx != -1) { sIdx = pIdx < aIdx ? pIdx : aIdx; } else if (pIdx != -1) { sIdx = pIdx; } else if (aIdx != -1) { sIdx = aIdx; }
    ...
    if (!hasMoneyContent) {
      ...
      if (mIdx != -1) {
        String moneyPart = suffix.substring(mIdx); final prefs = await SharedPreferences.getInstance();
    ...
    for (String n in rawNumbers) {
      if (n.trim().length != expLen) {
        _showError("Error: ${n.trim()} debe tener $expLen dígitos");
        return;
      }
    }
    double bankTopeBola = _parseLimit(_currentPlan["tope_bola"]), bankTopeParle = _parseLimit(_currentPlan["tope_parlet"]), bankTopeCentena = _parseLimit(_currentPlan["tope_centena"]);
    ...
    if (_activeField.value == 1) {
      ...
      // 1. Verificar historial en Lista para decidir si desglosamos
      double maxAcL = 0;
      for (String p in subPairs) {
        double ac = await _getAcumulado(p, "PARLE", destino: "LISTA");
        if (ac > maxAcL) maxAcL = ac;
      }
      
      if (maxAcL == 0) {
        // No hay historial: Intentar mantener el formato de grupo compacto
        double fillL = vM < topeParle ? vM : topeParle;
        if (fillL > 0) toLista.add({"n": rawNumbers.join('-'), "v": "(${fillL.toStringAsFixed(fillL % 1 == 0 ? 0 : 2)})"});
        
        double remaining = vM - fillL;
        if (remaining > 0.01) {
          double topeBoteP = (_personalTopes["parle_bote"] ?? _parseLimit(_currentPlan["tope_bote_parlet"])).clamp(0, _parseLimit(_currentPlan["tope_bote_parlet"]));
          // Verificar historial en Bote para el remanente
          double maxAcB = 0;
          for (String p in subPairs) {
            double ac = await _getAcumulado(p, "PARLE", destino: "BOTE");
            if (ac > maxAcB) maxAcB = ac;
          }
          
          if (maxAcB == 0) {
            double fillB = remaining < topeBoteP ? remaining : topeBoteP;
        ...
        // Existe historial previo: Desglose individual para respetar topes de cada pareja
        if (maxAcL + vM <= topeParle) {
          toLista.add({"n": rawNumbers.join('-'), "v": suffix});
        } else {
          for (String pair in subPairs) {
            double pairVM = vM, acL = await _getAcumulado(pair, "PARLE", destino: "LISTA"), dispL = (topeParle - acL).clamp(0.0, topeParle), fillL = pairVM < dispL ? pairVM : dispL;
            if (fillL > 0) toLista.add({"n": pair, "v": "(${fillL.toStringAsFixed(fillL % 1 == 0 ? 0 : 2)})"});
            double remaining = pairVM - fillL;
            if (remaining > 0.01) {
      ...
      // --- BOLA Y CENTENA ---
      // Forzar desglose individual (uno debajo del otro)
      double vF = 0, vC = 0, vM = 0;

      for (String n in rawNumbers) {
        String nL = n.trim();
        if (_activeField.value == 0) {
          List<double> ams = RecaudacionService.extractBolaAmounts(raw);
          vF = ams[0]; vC = ams[1];
          double aLF = await _getAcumulado(nL, "FIJO", destino: "LISTA"), 
                 aLC = await _getAcumulado(nL, "CORRIDO", destino: "LISTA"), 
                 dLF = (topeBola - aLF).clamp(0.0, topeBola), 
                 dLC = (topeBola - aLC).clamp(0.0, topeBola), 
                 fL = vF.clamp(0.0, dLF), 
                 cL = vC.clamp(0.0, dLC), 
                 fR = vF - fL, 
                 cR = vC - cL;
          ...
          if (fL > 0.01 || cL > 0.01) { 
            String sF = (vF > 0) ? fL.toStringAsFixed(fL % 1 == 0 ? 0 : 2) : 'X', 
                   sC = (vC > 0) ? cL.toStringAsFixed(cL % 1 == 0 ? 0 : 2) : 'X'; 
            toLista.add({"n": nL, "v": "($sF)($sC)"}); 
          }
          
          if (fR > 0.01 || cR > 0.01) {
            double acBF = await _getAcumulado(nL, "FIJO", destino: "BOTE"), 
                   acBC = await _getAcumulado(nL, "CORRIDO", destino: "BOTE"), 
                   tB = (_personalTopes["bola_bote"] ?? _parseLimit(_currentPlan["tope_bote_bola"])).clamp(0, _parseLimit(_currentPlan["tope_bote_bola"])),
                   dispBF = (tB - acBF).clamp(0.0, tB), 
                   dispBC = (tB - acBC).clamp(0.0, tB), 
                   fB = fR.clamp(0.0, dispBF), 
                   cB = cR.clamp(0.0, dispBC);

            if (fB > 0.01 || cB > 0.01) {
              String sF = (vF > 0) ? fB.toStringAsFixed(fB % 1 == 0 ? 0 : 2) : 'X', 
                     sC = (vC > 0) ? cB.toStringAsFixed(cB % 1 == 0 ? 0 : 2) : 'X';
          ...
          double aL = await _getAcumulado(nL, "CENTENA", destino: "LISTA"), 
                 dL = (topeCentena - aL).clamp(0.0, topeCentena), 
                 mL = vM.clamp(0.0, dL), 
                 mR = vM - mL;
                 
          if (mL > 0.01) toLista.add({"n": nL, "v": "(${mL.toStringAsFixed(mL % 1 == 0 ? 0 : 2)})"});
          
          if (mR > 0.01) {
            double acB = await _getAcumulado(nL, "CENTENA", destino: "BOTE"), 
                   tB = (_personalTopes["centena_bote"] ?? _parseLimit(_currentPlan["tope_bote_centena"])).clamp(0, _parseLimit(_currentPlan["tope_bote_centena"])), 
                   dispB = (tB - acB).clamp(0.0, tB), 
    ...
    if (toBote.isNotEmpty || rejected.isNotEmpty) {
      if (!mounted) return;
      List<Widget> content = [];
      if (toLista.isNotEmpty) {
        content.add(const Text("En la LISTA:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)));
      ...
      if (toBote.isNotEmpty) {
        content.add(const Text("Al BOTE (Exceso):", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)));
        for (var x in toBote) {
          content.add(Padding(
            padding: const EdgeInsets.only(left: 8.0, top: 4.0),
            child: _jugadaVerticalDisplay(x['n'] ?? '', x['v'] ?? '', const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87), isParle: tipo == "PARLE"),
    ...
    for (var x in toLista) {
      finalRows.add({'banco_id': bancoId, 'listero_pin': _listeroPin, 'tipo': tipo, 'valor': "${x['n']}${x['v']}", 'destino': 'LISTA', 'seccion': _activeSeccion, 'fecha': _activeFecha, 'loteria': _activeLoteria});
    }
    for (var x in toBote) {
    ...
    debugPrint("[S-RECORD DEBUG] Insertando lote: LISTA=${toLista.length}, BOTE=${toBote.length}");
    ...
    setState(() {
      _lastAddedCol = _activeField.value;
      _lastAddedCount = finalRows.length;
    });
    _scrollToHistoryTop(_activeField.value);
    inNot.value = '';
    } finally {
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Expanded(
              child: Text(
                count == 1 ? "ELIMINAR JUGADA" : "ELIMINAR $count JUGADAS",
        ...
        content: Text(
          count == 1
              ? "¿Está seguro de eliminar esta jugada permanentemente?\n\nEsta acción borrará el registro del teléfono y de la nube."
              : "¿Está seguro de eliminar permanentemente las $count jugadas seleccionadas?\n\nEsta acción borrará los registros del teléfono y de la nube.",
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(count == 1 ? "Jugada eliminada permanentemente" : "$count jugadas eliminadas permanentemente"),
            backgroundColor: Colors.green,
          )
        );
      }
    } catch (e) {
      debugPrint("Error deleting jugadas: $e");
  ...
  Future<void> _syncParte() async {
    ...
    if (tiro == null) return;
    final customLimites = await _db.getLimites(bancoId: bancoId, loteria: _activeLoteria); 
    final jl = await _db.getJugadasCompletas(_listeroPin, destino: 'LISTA', seccion: _activeSeccion, fecha: _activeFecha, bancoId: bancoId), jb = await _db.getJugadasCompletas(_listeroPin, destino: 'BOTE', seccion: _activeSeccion, fecha: _activeFecha, bancoId: bancoId);
  ...
  void _showSpecialOptions(String d) {
    ...
    if (d == 'AL') {
      showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Padding(padding: EdgeInsets.all(16), child: Text("Opciones Especiales", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.orange))),
        ListTile(leading: const Icon(Icons.copy_all, color: Colors.orange), title: const Text('Parejas (00-11-22...)'), onTap: () { _genSeries('PAREJAS', ''); Navigator.pop(ctx); }),
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      Padding(padding: const EdgeInsets.all(16), child: Text("Series para $d", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.lightBlue))),
                                ...
                                if (_tiroResult != null && pr > 0)
                                  Container(
                                    ...
                                    child: Row(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        Icon(coverage['completo'] ? Icons.verified : Icons.warning_amber_rounded, color: coverage['color'], size: 12),
                                        const SizedBox(width: 6),
                                        Text(
                                          coverage['mensaje'],
                                          style: TextStyle(color: coverage['color'], fontWeight: FontWeight.w900, fontSize: 9),
                                        ),
                                      ],
                                    ),
                                  ),
  ...
  Widget _buildTiroPublicadoPanel() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.blue.shade50, border: Border(top: BorderSide(color: Colors.blue.shade100, width: 0.5))),
      child: FittedBox(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text("TIRO GANADOR:", style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, color: Colors.blue.shade700, letterSpacing: 0.5)),
            const SizedBox(width: 12),
            _tiroBall(_tiroResult?['n1'] ?? '?', Colors.white, Colors.blue.shade700, isLarge: true),
            const SizedBox(width: 8),
            _tiroBall(_tiroResult?['n2'] ?? '?', Colors.white, Colors.blue.shade800),
            const SizedBox(width: 6),
            _tiroBall(_tiroResult?['n3'] ?? '?', Colors.yellowAccent, Colors.blue.shade900),
          ],
        ),
      ),
    );
  ...
  Widget _tiroBall(String n, Color bg, Color border, {bool isLarge = false}) => Container(
    ...
    child: Center(child: Text(n, style: TextStyle(fontSize: isLarge ? (n.length > 2 ? 16 : 20) : (n.length > 2 ? 12 : 14), fontWeight: FontWeight.w900, color: isLarge ? Colors.blue : Colors.black, fontFamily: 'monospace'))),
  );
  ...
  Widget _headerCell(String l, bool a, int f) => Expanded(flex: f, child: Text(l, textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: a ? Colors.blue.shade900 : Colors.blue.shade400)));
  ...
  Widget _jugadaVerticalDisplay(String nums, String money, TextStyle baseStyle, {bool isParle = false}) {
    List<String> rawNumbers = nums.split('-').where((s) => s.trim().isNotEmpty).toList();
    
    if (!isParle && rawNumbers.length > 1) {
      // En BOLA/CENTENA, si hay varios números en la jugada (Terminales/Comensales), se muestran uno debajo del otro
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisSize: MainAxisSize.min,
        children: rawNumbers.map((n) {
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 1.0),
            child: Row(
              children: [
                Text(n.trim(), style: baseStyle),
                const SizedBox(width: 6),
                Expanded(
                  child: Text.rich(
                    TextSpan(children: _richTextSpans(money, baseStyle.copyWith(fontSize: (baseStyle.fontSize ?? 14) - 2, fontWeight: FontWeight.normal))),
        ...
        builder: (ctx, act, _) => GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Center(
              child: Text(
                l,
                style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: act == i ? Colors.white : Colors.white70),
              ),
            ),
          ),
        ),
        ...
        child: Center(
          child: Text(k, 
            style: TextStyle(
              fontSize: 20, 
              fontWeight: FontWeight.w900, 
              color: isSpecial ? Colors.white : Colors.blue.shade900
            )
          ),
        ),
      ),
  ...
  Widget _tRow(String l, ValueNotifier<List<Map<String, dynamic>>>? n, {bool isL = false, bool isPr = false, bool isBal = false}) => ListenableBuilder(listenable: Listenable.merge([_limpioTotal, _premiosTotal, _bolaItems, _parleItems, _centenaItems]), builder: (ctx, _) {
    ...
    if (isPr) return _tTxt(l, pr, c: Colors.redAccent);
    if (isBal) return _tTxt(bal >= 0 ? 'Pierde' : 'Gana', bal.abs(), c: bal >= 0 ? Colors.greenAccent : Colors.redAccent);
    
    // For individual category brutos, we still calculate locally for immediate feedback
    ...
    child: Row(
      ...
      children: [
        ...
        Flexible(
          child: FittedBox(
            fit: BoxFit.scaleDown,
            child: Text(RecaudacionService.formatMoney(v), 
  ...
  Widget _buildDrawer() {
    ...
    Expanded(child: ListView(padding: EdgeInsets.zero, children: [
      _dIt(context, Icons.list_alt, 'LISTA', null, isSel: true), 
      _dIt(context, Icons.savings_outlined, 'BOTE', null), 
      _dIt(context, Icons.emoji_events_outlined, 'DETALLE PREMIOS', const PremiosScreen()), 
      _dIt(context, Icons.history_edu_rounded, 'MIS PARTES', const MisPartesScreen()), 
      _dIt(context, Icons.mail_outline, 'BUZÓN DE MENSAJES', const BuzonNotificacionesScreen()),
      _dIt(context, Icons.block_flipped, 'NÚMEROS LIMITADOS', NumerosLimitadosScreen()), 
      _dIt(context, Icons.tune_rounded, 'AJUSTE DE TOPES', const AjusteTopesListeroScreen()), 
      const Divider(), 
          ...
          child: Column(
            children: [
              ...
              FittedBox(
                fit: BoxFit.scaleDown, 
                child: Text(
                  "\$${RecaudacionService.formatMoney(saldo.abs())}", 
                  style: TextStyle(
